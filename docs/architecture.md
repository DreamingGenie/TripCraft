# 아키텍처 · 핵심 기술 노트 — TripCraft

> **목적**: 프로젝트의 핵심 알고리즘·설계 결정을 한 문서에서 조망.
> **구성**: 주제별 요약 + 상세 원문 링크(`features/`). 상세 분석은 각 원문 문서가 보유.
...
| # | 주제 | 핵심 도전 | 적용 패턴 | 원문 |
|---|------|-----------|-----------|------|
| 1 | 멀티모달 이동시간 | 외부 API 4종 조합 + 호출 제한 | 다중 호출 오케스트레이션 + 다층 캐시 | [transit-routing.md](features/transit-routing.md) |
| 2 | 실시간 협업 동시성 | 동시 편집 데이터 유실 방지 | 낙관적 락 + grab 게이트 + 행 직렬화 | [realtime-collab.md](features/realtime-collab.md) |
| 3 | 외부 데이터 동기화 | TourAPI 대량·증분 적재 | 배치 + 증분(modifiedtime) + 폴리라인 캐시 | (본 문서 §3) |
| 4 | 목록 조회 성능 | 프로필 이미지 N+1 | 파생 테이블 LEFT JOIN | [perf-profile-image.md](features/perf-profile-image.md) |
| 5 | 이미지/첨부 관리 | 다형 참조 + 생명주기 | `attach` target/target_id + draft 정리 | [image-lifecycle.md](features/image-lifecycle.md) |
...
> 참고: 원문 `features/image-lifecycle.md`는 초기 MVP 시점 작성이라 "컨트롤러 직접 I/O·DB 미추적"으로 기술돼 있으나, **현재 코드는 `FileStorageService` 위임 + `attach` draft 추적이 적용된 상태**다. 남은 과제: magic bytes 검증, 고아 파일 정리 스케줄러.
