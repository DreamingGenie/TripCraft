...
>
> **v2 변경점**: 원본 `화면설계서.md`(텍스트 명세)는 그대로 보존하고, 본 v2에 **핵심 5종 화면 와이어프레임(§6)** 을 추가했다.
...
> 중간 충실도(mid-fidelity) 목업. 색상은 디자인 시스템 토큰(purple `#534AB7`, teal `#0F6E56`, pink `#993556`, blue `#185FA5`)을 따른다.
> ID 표기는 `tripcraft_figma_spec_v0.2.md` 의 화면 분류(M/A/B/C/D)와 일치.

### 화면 M — 일정 생성 모달 (ScheduleModal)

여행 제목·기간(시작/종료일)·인원을 입력해 새 일정을 생성하는 모달.

![화면 M — 일정 생성 모달](wireframes/screen-M-create-modal.svg)

### 화면 A — 관광지 탐색 (ExploreView · `/discover`)

좌측 검색·지역/시군구/카테고리 필터·결과 목록, 우측 네이버 지도(마커 핀·InfoWindow). 카드의 「＋ 일정에 추가」로 후보군 등록.

![화면 A — 관광지 탐색](wireframes/screen-A-explore.svg)

### 화면 B — 내 일정 작업실 (PlanView / ScheduleBoard)

상단 툴바(일정 선택·협업 아바타·공유/협업자), 좌측 후보군 사이드바(도시별·드래그 소스·삭제존), 중앙 Day × 시간 그리드(확정 블록 + TransitPill), 우측 경로 지도 패널. 다른 협업자의 실시간 커서가 함께 표시된다.

![화면 B — 내 일정 작업실](wireframes/screen-B-scheduleboard.svg)

### 화면 C — 커뮤니티 (CommunityView)

정렬(최신/인기)·검색·글쓰기, PostCard 그리드, 우측 공지·인기 게시글 사이드바.

![화면 C — 커뮤니티](wireframes/screen-C-community.svg)

### 화면 D — 로그인 / 회원가입 (AuthView)

로그인/회원가입 탭, 이메일·비밀번호 폼, 카카오 시작 버튼.

![화면 D — 로그인/회원가입](wireframes/screen-D-auth.svg)
