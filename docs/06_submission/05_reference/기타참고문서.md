...
| Frontend | Vue.js 3 + Vite · Pinia · Vue Router · vue-draggable · Tiptap(에디터) |
...
- **인증 흐름**: 로그인 → JWT 발급(Access 헤더 / Refresh 관리) → `JwtAuthenticationFilter`가 매 요청 검증 → 만료 시 `/api/auth/refresh`로 재발급.
...
- **Git**: GitHub Flow 기반. `main` ← `develop` ← `feature/{도메인-기능}`·`fix/*`·`docs/*`. `main` 직접 push 금지, 1회 이상 리뷰 후 머지.
...
### 5.2 드래그앤드롭 타임라인
- vue-draggable 기반. 후보군 → Day 타임라인 드롭 시 `trip_block` 생성, 체류시간 핸들로 30분 스냅.
