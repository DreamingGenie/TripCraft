...
> 도메인 테이블 + 참조/설정 테이블. 모든 FK의 삭제 정책(CASCADE / SET NULL / RESTRICT)을 관계 라벨에 표기.
> 한 장에 모든 테이블을 그리면 가독성이 떨어져, **도메인별 4개 ERD**로 나눠 표기한다. (타 도메인에서 참조만 되는 테이블은 이름만 표시)
...
> `TRANSIT_CACHE`·`LANE_POLYLINE`·`SYSTEM_CONFIG`는 FK 없이 좌표 키·설정 키로 독립 운용되는 캐시/설정 테이블이다.
...
> Mermaid 표기상 핵심 컬럼만 발췌했다. 전체 DDL·인덱스·트리거·시드 데이터는 스키마 정본(`schema.sql`)을 따른다.
