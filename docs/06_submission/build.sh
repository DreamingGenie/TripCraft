#!/usr/bin/env bash
# TripCraft 최종 산출물 변환 스크립트
# Markdown/Mermaid 정본 → PDF · PPTX
...
npx -y @marp-team/marp-cli ppt/slides.md --pdf  --allow-local-files -o "$OUT/TripCraft_발표자료.pdf"  || echo "  [경고] PDF 변환 실패"
npx -y @marp-team/marp-cli ppt/slides.md --pptx --allow-local-files -o "$OUT/TripCraft_발표자료.pptx" || echo "  [경고] PPTX 변환 실패"
