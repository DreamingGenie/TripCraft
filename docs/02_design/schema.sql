...
-- initdb 적용 시 클라이언트 접속 charset 고정 — 없으면 sido·sigungu 한글 INSERT가
-- 이중 인코딩되어 저장됨(예: '서울'→'ì„œìš¸'). 관광지 seed(.sql.gz)도 동일 선언 보유.
SET NAMES utf8mb4;
...
-- ---------------------------------------------
-- 17. 방문 지도 표지 사진 (member_map_cover)
-- 마이페이지 방문 지도에서 지역별 표지 사진 + crop(초점/확대) 저장.
-- 표지는 여행이야기(후기)의 임의 사진을 고르거나 직접 업로드 → 지도 전용 사본으로 복사 저장
-- (image_url/host_path 소유). 출처 글 삭제(SET NULL)와 무관하게 지도 사진은 유지된다.
-- 미선택 지역은 앱 레이어에서 최신 후기 대표사진을 기본 표시(이 테이블엔 행 없음).
-- region_level: 현재 'SIDO'만 사용(시군구 'SIGUNGU' 확장 대비).
-- → migration_member_map_cover_v1.sql 참조
-- ---------------------------------------------
